...
import type { NewsArticle, NewsCategory, NewsArticleInput } from '@/types/admin'
...
export default function NewsEditPage() {
  ...
  useEffect(() => {
    const fetchArticle = async () => {
      try {
        const docSnap = await getDoc(newsDoc(articleId))
        if (!docSnap.exists()) {
          setNotFound(true)
          setLoading(false)
          return
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await updateDoc(newsDoc(articleId), {
        ...data,
        updated_at: now,
      })
  ...
  if (notFound) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <h2 className="text-xl font-bold text-gray-900 mb-2">뉴스를 찾을 수 없습니다</h2>
